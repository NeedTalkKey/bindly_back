...
router.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    // 2) 대화 내역 정규화
    const { normalizedText, speakerMapping, conversationMapping } =
      normalizeKakaoText(fileContent);
    console.log("normalizedText:", normalizedText);
    console.log("speakerMapping:", speakerMapping);
    console.log("conversationMapping:", conversationMapping);

    // 모델 엔드포인트
    const relationshipModelUrl =
      "https://api-inference.huggingface.co/models/kelly9457/bindly-R";
    const intimacyModelUrl = `${config.hosting.fastapi_ip}:${config.hosting.model_port}/predict`; // FastAPI 통합 서버에서 /predict 제공
    const empathyModelUrl = `${config.hosting.fastapi_ip}:${config.hosting.model_port}/empathy`;
    console.log("empathyModelUrl: ", empathyModelUrl);
    ...
    const empathyMesurePromise = fetch(empathyModelUrl, {
      ...
      body: JSON.stringify({ corpus: normalizedText, speaker: speakerMapping }),
    }).then(async (res) => {
      if (!res.ok) {
        const errText = await res.text();
        throw new Error(`공감형/직설적 모델 에러: ${res.status} - ${errText}`);
      }
      return res.json();
    });
    ...
    const rawScore = intimacyChunkResults.length
      ? sumRaw / intimacyChunkResults.length
      : 0;
    const scaledScore = intimacyChunkResults.length
      ? sumScaled / intimacyChunkResults.length
      : 0;
